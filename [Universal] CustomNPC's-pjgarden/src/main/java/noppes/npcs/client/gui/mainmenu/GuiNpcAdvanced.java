...
public class GuiNpcAdvanced extends GuiNPCInterface2 implements IGuiData
{
    ...
    public void initGui(){
    	...
    	this.addButton(new GuiNpcButton(8, guiLeft + 85, guiTop + 20,155,20, new String[]{"role.none","role.trader","role.follower","role.bank","role.transporter", "role.mailman", NoppesStringUtils.translate("role.companion", "(WIP)"), "role.saleshop", "role.purchaseshop"},npc.advanced.role.ordinal()));
	...
	public void setGuiData(NBTTagCompound compound) {
		if(compound.hasKey("RoleData")){
			...
			else if(npc.advanced.role == EnumRoleType.SaleShop)
				NoppesUtil.requestOpenGUI(EnumGuiType.SaleShopSetup);
			else if(npc.advanced.role == EnumRoleType.PurchaseShop)
				NoppesUtil.requestOpenGUI(EnumGuiType.PurchaseShopSetup);
		}
