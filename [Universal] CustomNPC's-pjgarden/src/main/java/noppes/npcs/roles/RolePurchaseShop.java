...
public class RolePurchaseShop extends RoleInterface {

	private String shopName;
	private ShopManager manager;
	
	public RolePurchaseShop(EntityNPCInterface npc) {
		super(npc);

		shopName = "purchase_" + npc.getUniqueID().toString().replace("-", "").substring(0, 5) + "_" + npc.getCommandSenderName();
		manager = new ShopManager(shopName, ShopType.PURCHASE);
	}
