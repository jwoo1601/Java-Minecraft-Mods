...
public class PacketPlayerStat implements IMessage
{
	...
	public void fromBytes(ByteBuf buf) {
		...
		if (packetType == Type.REQUEST) {
			...
			char[] tmp = new char[(buf.readableBytes() - 13) / 2];
	...
	public void toBytes(ByteBuf buf) {
		...
		if (data != null) {
			...
			buf.writeBoolean(data.canClean);
		}
	}
...
//	=====================================
	...
	public IMessage onMessage(PacketPlayerStat message, MessageContext ctx) {
		...
		if (message.packetType == Type.RESPONSE) {
			...
			EntityPlayer cplayer = TtaemiriKingMod.proxy.getClientPlayer();
	...
	public static class ServerHandler implements IMessageHandler<PacketPlayerStat, PacketPlayerStat> {
